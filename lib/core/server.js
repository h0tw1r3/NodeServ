...
var NodeServ = function(config) {
	...
	var requestHandler = function(ap, req, res) {
	...
	// Listen on all relevant interfaces/ports.
	var binds = Array.isArray(config.bind) ? config.bind : [{port: 80}];
	...
	this.start = function(callbackFn) {
		async.map(binds, function(ap, callback) { 
			var server = http.createServer(requestHandler.bind(this, ap));
			...
			server.listen(ap.port, ap.host, callback.bind(this, null, server));
